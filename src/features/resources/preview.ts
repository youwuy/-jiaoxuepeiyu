...
export function resourcePreviewKind(resource?: PreviewableResource | null): ResourcePreviewKind {
  ...
  if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg'].includes(extension) || type.includes('图片') || enumType === 'IMAGE') {
    return 'image';
  }
  if (['mp4', 'webm', 'ogg', 'mov'].includes(extension) || type.includes('视频') || enumType === 'VIDEO') {
    return 'video';
  }
  if (['mp3', 'wav', 'm4a', 'aac', 'oga'].includes(extension) || type.includes('音频') || enumType === 'AUDIO') {
    return 'audio';
  }
  if (
