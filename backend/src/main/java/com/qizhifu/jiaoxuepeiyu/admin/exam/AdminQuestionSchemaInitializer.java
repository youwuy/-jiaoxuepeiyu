...
public class AdminQuestionSchemaInitializer implements ApplicationRunner {
    ...
    void ensureQuestionCompatibility() {
        ...
        if (operations.tableExists("exam_question")
                && !operations.columnExists("exam_question", "course_name")) {
            operations.execute("ALTER TABLE `exam_question` ADD COLUMN `course_name` VARCHAR(128) NULL AFTER `question_type`");
        }
