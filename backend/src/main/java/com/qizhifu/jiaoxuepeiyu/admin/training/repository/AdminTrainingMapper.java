...
public interface AdminTrainingMapper {
    ...
    @Select("<script>"
            ...
            + "<if test='publishStatus == \"UNPUBLISHED\"'>AND t.publish_status IN ('DRAFT', 'OFFLINE')</if> "
            + "<if test='publishStatus != null and publishStatus != \"UNPUBLISHED\"'>AND t.publish_status = #{publishStatus}</if> "
